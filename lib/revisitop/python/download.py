...
def download_datasets(data_dir):
    ...
    for di in range(len(datasets)):
        ...
        if dataset == 'roxford5k':
            ...
            raise ValueError('Unknown data: {}!'.format(dataset))
        ...
        if not os.path.isdir(dst_dir):
            ...
            for dli in range(len(dl_files)):
                ...
                print('>> Downloading data {} archive {}...'.format(dataset, dl_file))
                os.system('wget {} -O {}'.format(src_file, dst_file))
                print('>> Extracting data {} archive {}...'.format(dataset, dl_file))
                # create tmp folder
                ...
                print('>> Extracted, deleting data {} archive {}...'.format(dataset, dl_file))
        ...
        if not os.path.exists(gnd_dst_file):
            print('>> Downloading data {} ground truth file...'.format(dataset))
            os.system('wget {} -O {}'.format(gnd_src_file, gnd_dst_file))


def download_distractors(data_dir):
    """
    DOWNLOAD_DISTRACTORS Checks, and, if required, downloads the distractor data.

    download_distractors(DATA_ROOT) checks if the distractor data exist.
    ...
    if not os.path.isdir(dst_dir):
        ...
        for dfi in range(nfiles):
            ...
            if os.path.exists(dst_file):
                print('>> [{}/{}] Skipping data {} archive {}, already exists...'.format(dfi+1, nfiles, dataset, dl_file))
            else:
                while 1:
                    try:
                        print('>> [{}/{}] Downloading data {} archive {}...'.format(dfi+1, nfiles, dataset, dl_file))
        ...
        for dfi in range(nfiles):
            dl_file = dl_files.format(dfi+1)
            dst_file = os.path.join(dst_dir_tmp, dl_file)
            print('>> [{}/{}] Extracting data {} archive {}...'.format(dfi+1, nfiles, dataset, dl_file))
            tar = tarfile.open(dst_file)
            tar.extractall(path=dst_dir_tmp)
            tar.close()
            print('>> [{}/{}] Extracted, deleting data {} archive {}...'.format(dfi+1, nfiles, dataset, dl_file))
            os.remove(dst_file)
        # rename tmp folder
        ...
        gnd_dst_file = os.path.join(gnd_dst_dir, gnd_dl_file)
        if not os.path.exists(gnd_dst_file):
            print('>> Downloading data {} image list file...'.format(dataset))
...
def download_features(data_dir):
    ...
    for di in range(len(datasets)):
        ...
        if not os.path.exists(feat_dst_file):
            print('>> Downloading data {} features file {}...'.format(dataset, feat_dl_file))
